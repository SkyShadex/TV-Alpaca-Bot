...
def calcPerformance():
    ...
    if activity_df is not None:
        ...
        price_filtered = activity_df.loc[
            (activity_df["price"] < 1.0) & (activity_df["side"] == "buy")
        ]
        ...
        try:
            ...
            grouped_data = (
                pnl_df.groupby("base_symbol")
                .agg({"buy_sum": "sum", "sell_sum": "sum", "Avg_Price": "mean"})
                .reset_index()
            )
            grouped_data["Returns"] = (
                grouped_data["sell_sum"] / grouped_data["buy_sum"]
            ) - 1
...
def plotOptionsMetrics(grouped_data):
    ...
    grouped_data["Avg_Price_Quantiles"] = grouped_data["Avg_Price_Quantiles"].apply(
    ...
    fig.suptitle(
        f"Options Performance Analysis. Estimated P/L: {grouped_data.net_PnL.sum():.2f}",
        fontsize=16,
    )
...
def collectOrders():
    ...
    order_params = GetOrdersRequest(
        status="closed", until=end_date, limit=500, nested=False
    )  # TODO: Clean later
    all_orders = []
    ...
    try:
        while True:
            ...
            df_data = [
                vars.extract_order_response(order)
                for order in orders
            ...
            if df_data:
                created_at_values = pd.to_datetime(
                    [order["created_at"] for order in df_data], utc=True
                )
...
def dataCrunch(plot_filename):
    ...
    bars = plt.barh(
        ...
        color=plt.cm.get_cmap(cmap)(
            0.5 + df_sorted["pnl"] / df_sorted["pnl"].max() / 2
        ),
    )  # TODO: Clean later
