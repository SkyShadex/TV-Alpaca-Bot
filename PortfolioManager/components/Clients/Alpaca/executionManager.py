...
def entryOrder(symbol,price,orderID,side='buy',isMarketOrder=False,weight=1.0,quantity=0.0,client=api.client['DEV']):
    if price == 0:
...
def optionsOrder(symbol,price,client=api.client['DEV'],orderID="",side='buy',quantity=1,isMarketOrder=False,weight=1.0,tif='day',bet=100.0,vol_limit=5):
    if isMarketOrder:
        ...
        if side != 'sell': 
            ...
            qty = min(max(math.floor(portfolio_Corrected / (100 * price)),1),vol_limit) if price != 0 else 1
...
class SkyOrder:
    def __init__(self, client, symbol: str, order_type: str, **kwargs):
        ...
        self.client = 'LIVE' if client is api.client['LIVE'] else 'DEV' if client is api.client['DEV'] else 'Err'
        ...
        if 'None' in self.asset_class:
            self.asset_class = 'us_option' if len(symbol) > 8 else 'us_equity' # be aware that api offers crypto, but it's not available for me
...
class ExecutionManager(threading.Thread):
            ...
            try:
                ...
                if order_id:
                    ...
                    if order_data:
                        ...
                        order_data['client'] = api.client['LIVE'] if order_data['client'] == 'LIVE' else api.client['DEV'] if order_data['client'] == 'DEV' else 'Err'
    ...
    def exitOrder(self,order_data):  
        ...
        try:
            ...
            if symbol in pos_all['symbol'].values:
                match = pos_all.loc[pos_all.symbol == order_data['symbol']].copy().iloc[0]
                if abs(match.unrealized_plpc) > 0.05: # to reduce turnover
                    res = order_data['client'].close_position(symbol)
            print(f"[{order_data['client_label']}]: {order_data['order_type']} : {order_data['symbol']} {order_data['order_memo']}")
            time.sleep(2)
    ...
    def entryOrder(self,order_data):
        ...
        side = order_data['side']
        price = float(order_data['price'])
        quantity = float(order_data['quantity']) if 'sell' not in side else min(int(order_data['quantity']),1)
        order_type = order_data['order_type']
        client_order_id = order_data["order_id"]
        ...
        try:
            ...
            if toChildOrders:
                ...
                for i in range(int(intervals_remaining)):
                    ...
                    child_quantity = int(math.ceil(total_quantity * percentage_per_interval / 100)) if 'short' in str(pos['side']).lower() else round(total_quantity * percentage_per_interval / 100, 9)
                    ...
                    # entryOrder [symbol,price,client_order_id,side=side,isMarketOrder=True,weight=1.0,quantity=quantity,client=order_data['client']]
                    run_date = current_time + datetime.timedelta(minutes=interval * (i + 1))
                    scheduler.add_job(
                ...
                if total_quantity > 0:
                    ...
                    run_date = current_time + datetime.timedelta(minutes=interval * int(intervals_remaining))
                    scheduler.add_job(
                        order_data['client'].submit_order,
                        'date',
                        run_date=run_date,
