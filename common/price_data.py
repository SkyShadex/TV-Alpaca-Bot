...
def get_latest_quote(symbol, mode="equity"):
    ...
    if isinstance(q, dict):
        for symbol, quote in q.items():
            ...
            bid_price = max(
                quote.bid_price, 2.2250738585072014e-308
            )  # reduce errors from bid_price being zero in live environments
...
def get_ohlc_alpaca(
    ...
    if isinstance(symbols, (pd.Series, list)):
        ...
        print(
            get_ohlc_alpaca.__name__, 82303579834986
        )  # TODO: Check if this is being used as a pd/list anywhere????
        if symCount >= 200:
        ...
        if useCache:
            ...
            if not old_price_data.empty:
                daterange = (
                    old_price_data["date"].max() - old_price_data["date"].min()
                ).days
                if daterange >= 100:
                    logging.debug(
                        f"OHLCV:{symbols[0]}:{str(timeframe)} [CACHED]", daterange
                    )
    ...
    for i, symbol in enumerate(symbols):
        ...
        if isCrypto:
            params = CryptoBarsRequest(
                symbol_or_symbols=symbol, start=start_date, timeframe=timeframe
            )
        ...
        try:
            ...
            ohlcv.set_index(
                pd.DatetimeIndex(ohlcv["date"], tz=UTC_TIMEZONE), inplace=True
            )
        ...
        if (
            days_collected <= lookback * 0.95 and symCount > 1 and date_err
        ):  # Filter Out Incomplete Data
            skipped_symbols.append(symbol)
            logging.warning(
                f"Expected {lookback*0.95:.0f} days of data, received {days_collected}"
            )
            continue
    ...
    if len(all_ohlcv) == 1:  # Single symbol request
        ...
        if oldFound:
            ...
            new_df.set_index(
                pd.DatetimeIndex(new_df["date"], tz=UTC_TIMEZONE), inplace=True
            )
...
def store_ohlcv_db(ohlcv, symbol, timeframe, cached=True):
    ...
    def store_ohlcv_TS(row):
        ...
        ts.add(
            f"TS-OHLCV:{row.symbol}:{str(timeframe)}",
            int(row.name.timestamp() * 1000),
            float(row.close),
            labels=labels,
    ...
    try:
        ...
        if price_data_old_json is not None:
            price_data_old = pd.read_json(
                StringIO(price_data_old_json), orient="records"
            )
            ...
            price_data_complete["date"] = pd.to_datetime(
                price_data_complete["timestamp"], utc=True
            )
            price_data_complete.set_index(
                pd.DatetimeIndex(price_data_complete["date"], tz=UTC_TIMEZONE),
                inplace=True,
            )
            price_data_complete.drop_duplicates(
            ...
            price_data_complete["isGap"] = ~(
                price_data_complete["close"]
                .shift(1)
                .between(price_data_complete["low"], price_data_complete["high"])
            )
            gappc = round(
                price_data_complete["isGap"].sum() / len(price_data_complete["isGap"]),
                4,
            )
            redis_client.zadd(
                f"asset:metrics:gappc", mapping={str(symbol): float(gappc)}
            )
...
def retrieve_ohlcv_db(symbol, timeframe):
    ...
    try:
        ...
        if price_data_old_json is not None:
            ...
            price_data_old["date"] = pd.to_datetime(
                price_data_old["timestamp"], utc=True
            )
            price_data_old.drop_duplicates(keep="last", inplace=True)
            price_data_old.set_index(
                pd.DatetimeIndex(price_data_old["date"], tz=UTC_TIMEZONE), inplace=True
            )
...
def randomStats(db_client: redis.Redis = None):
    ...
    metrics_df = pd.DataFrame(
        metrics.items(), columns=["symbol", "priceGaps_to_totalBars"]
    )
    metrics_df.set_index("symbol", inplace=True)
    metrics_df["priceGaps_to_totalBars"] = metrics_df["priceGaps_to_totalBars"].astype(
        float
    )
    return metrics_df
