...
def dataCrunch(plot_filename):
    ...
    for symbol in symbols:
        ...
        if net_qty != 0:
            ...
            total_sell_amount = (closed_orders[closed_orders['side'] == 'sell']['filled_qty'] * closed_orders[closed_orders['side'] == 'sell']['filled_avg_price']).sum()
            total_buy_amount = (closed_orders[closed_orders['side'] == 'buy']['filled_qty'] * closed_orders[closed_orders['side'] == 'buy']['filled_avg_price']).sum()
            total_sell_qty = sell_qty
            total_buy_qty = buy_qty

            pnl = total_sell_amount - total_buy_amount
            if total_sell_amount !=0:
                closed_positions.append((symbol, net_qty, total_sell_qty, total_buy_qty, pnl, total_sell_amount, total_buy_amount))
            
        else:
            # No open position, calculate P/L for all orders
            total_sell_amount = (symbol_df[symbol_df['side'] == 'sell']['filled_qty'] * symbol_df[symbol_df['side'] == 'sell']['filled_avg_price']).sum()
            total_buy_amount = (symbol_df[symbol_df['side'] == 'buy']['filled_qty'] * symbol_df[symbol_df['side'] == 'buy']['filled_avg_price']).sum()
            total_sell_qty = sell_qty
            total_buy_qty = buy_qty
    ...
    closed_positions_df = pd.DataFrame(closed_positions, columns=['symbol', 'pnl',"net_qty","total_sell_qty","total_buy_qty", 'total_sell_amount', 'total_buy_amount'])
